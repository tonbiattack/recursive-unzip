...
|外部サービス・通信|不要|
|ZIPの削除|Explorerメニューでは展開成功した内部ZIPだけを削除し、選択した最上位ZIPは保持。CLIでは`--delete-nested-zip`または`--delete-zip`で指定|
...
|機能|実装内容|
|---|---|
|再帰展開|ネスト階層に固定上限を設けず、未処理の内部ZIPがなくなるまで反復処理します。Explorerメニューでは展開成功した内部ZIPを削除し、最上位ZIPは残します。|
...
|既存フォルダ|既定では同名ファイルを上書きします。スキップまたは別名フォルダ作成も選べます。|
|完了後の表示|CLIでは成功後にExplorerを開きます。Explorerメニューからの実行では追加のExplorerウィンドウを開きません。|
...
|`--delete-nested-zip`|展開成功後、内部ZIPだけを削除します。引数で指定した最上位ZIPは残します。Explorerメニューでは自動的に付与されます。|
...
既定では、`images.zip` の展開先 `images` が既に存在しても、そのフォルダを削除せずに展開します。ZIP内にある同じ相対パスのファイルは上書きし、ZIPに含まれない既存ファイルは残ります。

ZIPの中に含まれる内部ZIP（子ZIP）も、親の展開結果の一部として常に同じ上書き方式で展開します。したがって、`--skip-existing` と `--rename-existing` は最初に指定したZIPの展開先だけに適用され、内部ZIPの展開先には適用されません。
...
登録後はExplorerで1個以上のZIPを選び、右クリックして **ZIPを再帰展開** を選択します。展開に成功した内部ZIPは削除されますが、選択した最上位ZIPは残ります。Explorerメニューからの実行には`--no-open`が自動付与されるため、成功後に展開先をExplorerで開きません。Windows 11では、レジストリベースの静的verbが **その他のオプションを表示** のサブメニューに現れる場合があります。コマンド文字列では実行ファイルと`%1`を引用符で囲んでおり、空白を含むパスにも対応します。Windows Shellのcommand文字列では、スペースを含む可能性がある実行ファイル・引数を引用符で囲むことが推奨されています。[2]

> 複数選択には、複数項目を1回のプロセス起動で扱うverb用の `MultiSelectModel=Player` を登録します。個別ウィンドウを開く `Document` ではないため、複数ZIPを選んでもコマンドプロンプトは1つだけです。[3] レジストリベースの静的verbはWindows Shellの既定上限により、1回に100項目を超える選択では表示されない場合があります。[3] その場合はCLIから複数ZIPを直接指定してください。
...
|ZIP内ZIP|2階層・3階層の再帰展開、内部ZIPだけの削除|
...
|境界条件|空ZIP、`--delete-zip`、`--delete-nested-zip`|
