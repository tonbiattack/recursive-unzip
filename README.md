...
登録後はExplorerで1個以上のZIPを選び、右クリックして **ZIPを再帰展開** を選択します。展開に成功した内部ZIPは削除されますが、選択した最上位ZIPは残ります。Windows 11では、レジストリベースの静的verbが **その他のオプションを表示** のサブメニューに現れる場合があります。コマンド文字列では実行ファイルと`%1`を引用符で囲んでおり、空白を含むパスにも対応します。Windows Shellのcommand文字列では、スペースを含む可能性がある実行ファイル・引数を引用符で囲むことが推奨されています。[2]
